...
if [ -z "$ENVIRONMENT_NAME" ]; then
    echo "ERROR: Environment name not provided. Set AZURE_ENV_NAME or pass as first argument"
    exit 1
fi
...
if [ $? -ne 0 ] || [ -z "$ENV_JSON" ]; then
    echo "ERROR: Could not get environment info. Make sure you're in the right directory and environment is set."
    exit 1
fi
...
if command -v jq &> /dev/null; then
    ...
    BACKEND_SERVICE_URL=$(echo "$ENV_JSON" | jq -r '.SERVICE_BACKEND_SERVICE_ENDPOINT_URL // empty')
    ...
    BACKEND_SERVICE_URL=$(echo "$ENV_JSON" | grep -o '"SERVICE_BACKEND_SERVICE_ENDPOINT_URL"[^,]*' | cut -d'"' -f4)
...
echo "Backend Service: $BACKEND_SERVICE_URL"
...
if curl -s -I "$FRONTEND_URL" --max-time 10 | grep -q "200 OK"; then
    ...
    if echo "$CONTENT_TYPE" | grep -q "text/html"; then
        echo "SUCCESS: Frontend is responding with HTML"
    else
        echo "WARNING: Frontend responded but not with HTML content"
    fi
else
...
# Test APIs
declare -a apis=(
    "Backend Service:$BACKEND_SERVICE_URL/health"
)
...
for api in "${apis[@]}"; do
    ...
    if curl -s -I "$url" --max-time 10 | grep -q "200 OK"; then
        ...
        if echo "$CONTENT_TYPE" | grep -q "application/json"; then
            echo "SUCCESS: $name is responding with JSON"
        else
            echo "WARNING: $name responded but not with JSON (might be placeholder)"
    ...
    if command -v az &> /dev/null; then
        ...
        if [ "$REPOS" != "[]" ]; then
            ...
            if [ ${#MISSING_REPOS[@]} -eq 0 ]; then
                echo "SUCCESS: All expected container images found in registry"
            else
                echo "WARNING: Missing container images: ${MISSING_REPOS[*]}"
                echo "   This indicates the applications weren't built/pushed properly"
            fi
        else
            echo "WARNING: No repositories found in container registry"
        fi
    else
        echo "WARNING: Azure CLI not available to check container registry"
    fi
else
    echo "WARNING: Container registry name not found in environment"
fi

echo ""
echo "Validation Summary:"
echo "If you see WARNING or ERROR indicators above, try running:"
